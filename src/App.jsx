...
import {PrivateSale} from './data/PrivateSale';
import {OffMarketDeals} from './data/OffMarketDeals';
import {TehachapiBuy} from './pages/TehachapiBuy';
import {BakersfieldBuy} from './pages/BakersfieldBuy';
import {CaliforniaCityInvest} from './pages/CaliforniaCityInvest';
import {StalionSpringsCommunities} from './pages/StalionSpringsCommunities';
...
const pages={'/':{name:'Home',Component:Home},'/about':{name:'About',Component:About},'/guides':{name:'Guides',Component:GuidesIndex},'/blog':{name:'Blog',Component:BlogIndex},'/properties':{name:'Properties',Component:PropertiesPage},'/past-sales':{name:'Past Sales',Component:PastSalesPage},'/open-houses':{name:'Open Houses',Component:OpenHousesPage},'/moving-from-los-angeles-to-bakersfield':{name:'LA to Bakersfield',Component:MovingFromLosAngeles},'/contact':{name:'Contact',Component:Contact},'/home-value':{name:'Home Value',Component:HomeValue},'/hq':{name:'HQ',Component:Hq},'/why-tehachapi':{name:'Why Tehachapi',Component:WhyTehachapi},'/cheap-land-kern-county':{name:'Cheap Land',Component:CheapLandKernCounty},'/bakersfield-home-prices':{name:'Bakersfield Prices',Component:BakersfieldHomePrices},'/tehachapi-home-prices':{name:'Tehachapi Prices',Component:TehachapiHomePrices},'/tehachapi':{name:'Tehachapi Homes',Component:TehachapiBuy},'/bakersfield':{name:'Bakersfield Homes',Component:BakersfieldBuy},'/california-city':{name:'California City Land',Component:CaliforniaCityInvest},'/stallion-springs':{name:'Stallion Springs',Component:StalionSpringsCommunities},'/private-sale':{name:'Private Sale',Component:PrivateSale},'/off-market-deals':{name:'Off-Market Deals',Component:OffMarketDeals}};
export function App({initialPath,initialProperty,initialProperties=null,initialContent=null}={}){
 const path=(initialPath||window.location.pathname).replace(/\/+$/,'')||'/';
 const propertyMatch=path.match(/^\/property\/([^/]+)$/);
 const contentMatch=path.match(/^\/(guide|blog)\/[^/]+$/) ? 'content' : null;
 const page=pages[path]||(contentMatch?{name:'Content',Component:()=><ContentRouter path={path} initialContent={initialContent}/>}:(propertyMatch?{name:'Property',Component:PropertyDetailPage}:(servicePages[path]?{name:servicePages[path].name,Component:ServicePage}:undefined)));
 const [menu,setMenu]=useState(false);
 const [showVideo,setShowVideo]=useState(false);
 useEffect(()=>{try{setShowVideo(!localStorage.getItem('video_seen'))}catch{}},[]);
 ...
 useEffect(()=>{
  ...
  if(/^\/(property|guide|blog)\/[^/]+$/.test(path))return;
  const route=routes[path]||{title:'Page not found | Harbison Standard',description:'',robots:'noindex, follow'};
