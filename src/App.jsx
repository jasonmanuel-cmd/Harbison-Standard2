...
const pages={'/':{name:'Home',Component:Home},'/about':{name:'About',Component:About},'/properties':{name:'Properties',Component:Properties},'/contact':{name:'Contact',Component:Contact},'/hq':{name:'HQ',Component:Hq}};
export function App(){
 ...
 <footer className="site-footer"><div><p>Bakersfield · Tehachapi · Delano · Lake Isabella · Kern County</p><p className="footer-credentials"><a href={agent.phoneHref}>{agent.phone}</a> · <a href={'mailto:'+agent.email}>{agent.email}</a></p><p className="footer-credentials">{agent.name} · REALTOR® · Harbison Standard · DRE #{agent.license}</p><p className="footer-credentials">Equal Housing Opportunity</p></div><SocialLinks/></footer>
 <div className="mobile-contact" role="navigation" aria-label="Quick contact"><a href={agent.phoneHref}><Phone/>Call</a><a href={agent.smsHref}><ChatCircleText/>Text</a><a href="/contact"><Envelope/>Contact</a></div></>;
}
