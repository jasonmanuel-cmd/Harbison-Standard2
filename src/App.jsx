...
export function App({initialPath,initialProperty,initialProperties=null}={}){
 ...
 const page=pages[path]||(propertyMatch?{name:'Property',Component:PropertyDetailPage}:(servicePages[path]?{name:servicePages[path].name,Component:ServicePage}:undefined));
 ...
 return <><a className="skip" href="#main">Skip to content</a><header><a className="brand" href="/" aria-label="Harbison Standard home"><img src="/assets/logo.webp" alt="Harbison Standard — Real Estate and Investing"/></a><button className="menu-toggle" onClick={()=>setMenu(!menu)} aria-label={menu?'Close navigation':'Open navigation'} aria-expanded={menu} aria-controls="main-nav">{menu?<X/>:<List/>}</button><nav id="main-nav" className={menu?'open':''} aria-label="Main navigation">{[['/','Home'],['/#approach','Our Approach'],['/properties','Properties'],['/private-sale','Private Sale'],['/off-market-deals','Off-Market Deals'],['/moving-from-los-angeles-to-bakersfield','Relocate'],['/about','About'],['/contact','Contact']].map(([href,label])=><a key={href} className={href===path?'active':''} aria-current={href===path?'page':undefined} href={href} onClick={()=>setMenu(false)}>{label}</a>)}</nav><a className="gold header-cta" href="/contact">Let’s talk</a></header>
