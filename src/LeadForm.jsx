...
export function LeadForm({source,goalOptions=defaultGoals,messagePlaceholder="Your goals, price range, or questions…"}){
 ...
 useEffect(()=>{trackEvent('form_start',{form_name:'general_inquiry',lead_source:source});const goal=new URLSearchParams(window.location.search).get('goal');if(goals.some(g=>g.name===goal))setData(current=>({...current,goal}));},[source]);
 ...
 useEffect(()=>{if(previousStep.current!==step){title.current?.focus();trackEvent('form_step_change',{step,lead_source:source});}previousStep.current=step},[step,source]);
 const update=e=>{setData({...data,[e.target.name]:e.target.value});setNotice('')};
 const selectGoal=(goal)=>{trackEvent('goal_selected',{goal,lead_source:source});setData({...data,goal});};
