...
export function PastSalesPage(){
 ...
 {featured&&featured.images?.length>0&&<section className="portfolio-section page-wrap"><div className="section-heading"><div><p className="eyebrow">Featured Transaction</p><h2>The detail that <em>speaks.</em></h2></div></div><div className="featured-transaction"><article className="featured-property-compact"><Photo p={featured} featured/><div className="featured-property-info"><div><p className="eyebrow">Sold {new Date(featured.date+'T12:00:00').toLocaleDateString('en-US',{year:'numeric',month:'short'})}</p><h3>{featured.address}</h3><p className="property-location">{featured.city}, {featured.zip}</p><Facts p={featured}/>{featured.lot&&<p className="property-fact">Lot: {featured.lot}</p>}</div><div><strong className="transaction-price">{money(featured.price)}</strong>{featured.context&&<p>{featured.context}</p>}</div></div></article></div>{featured.images?.length>1&&<div className="featured-gallery-preview"><p className="eyebrow">{featured.images.length} photos</p><div className="gallery-grid">{featured.images.slice(0,6).map((src,i)=><div key={src+i} className="gallery-thumb"><img src={src} alt={`${featured.address} photo ${i+1}`} loading="lazy"/></div>)}</div>}</div>}</section>}
